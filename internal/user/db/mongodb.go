...
func (d db) Create(ctx context.Context, user user.User) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to crete user due to error: %v", err)
	}
...
func (d db) FindAll(ctx context.Context) (u []user.User, err error) {
	...
	if cursor.Err() != nil {
		return u, fmt.Errorf("failed to find all users from db due to error: %v", err)
	}
...
func (d db) Update(ctx context.Context, user user.User) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert hex to objectid. id: %s", user.ID)
	}
